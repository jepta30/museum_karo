...
<body class="bg-museum-bg text-gray-800 font-sans h-screen flex overflow-hidden">
        ...
        <div>
            ...
            <nav class="px-4 mt-2">
                @if(Auth::user()->peran === 'admin')
                    ...
                    <a href="{{ route('admin.users') }}" class="flex items-center gap-3 px-4 py-2.5 mt-1 text-sm font-medium rounded-md {{ request()->routeIs('admin.users') ? 'bg-[#f7f2ed] text-black border-r-4 border-[#5c1c16]' : 'text-gray-600 hover:bg-gray-50' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        Manajemen Akun
                    </a>
                    <a href="{{ route('admin.logs') }}" class="flex items-center gap-3 px-4 py-2.5 mt-1 text-sm font-medium rounded-md {{ request()->routeIs('admin.logs') ? 'bg-[#f7f2ed] text-black border-r-4 border-[#5c1c16]' : 'text-gray-600 hover:bg-gray-50' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                        Log Aktivitas
                    </a>
                    <a href="{{ route('admin.roles') }}" class="flex items-center gap-3 px-4 py-2.5 mt-1 text-sm font-medium rounded-md {{ request()->routeIs('admin.roles') ? 'bg-[#f7f2ed] text-black border-r-4 border-[#5c1c16]' : 'text-gray-600 hover:bg-gray-50' }}">
                ...
                @elseif(Auth::check() && Auth::user()->peran === 'kurator')
                    <a href="{{ route('curator.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('curator.dashboard') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm mt-1">
                        <!-- Ikon Dasbor -->
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                        Dasbor Utama
                    </a>
                    <a href="{{ route('curator.kurasi') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('curator.kurasi') || request()->routeIs('curator.edit') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm mt-1">
                        <!-- Ikon Verifikasi -->
                    ...
                    <a href="{{ route('curator.repository') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('curator.repository') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm mt-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                        Repositori Institusi
                    </a>
                    <a href="{{ route('curator.komentar') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('curator.komentar') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm mt-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                        Komentar Pengunjung
                        @php
                            $pendingCommentsCount = \App\Models\Komentar::where('status', 'menunggu')->count();
                        @endphp
                        @if($pendingCommentsCount > 0)
                            <span class="ml-auto bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $pendingCommentsCount }}</span>
                        @endif
                    </a>
                    <a href="{{ route('curator.katalog') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('curator.katalog') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm mt-1">
                        <!-- Ikon Katalog -->
                ...
                @elseif(Auth::check() && Auth::user()->peran === 'edukator')
                    <nav class="space-y-1">
                        <a href="{{ route('educator.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('educator.dashboard') ? 'bg-[#fbf7f5] text-museum-red font-semibold border-l-4 border-museum-red' : 'text-gray-500 hover:text-museum-red hover:bg-[#fbf7f5]' }} rounded-md transition group">
                            <svg class="w-5 h-5 {{ request()->routeIs('educator.dashboard') ? 'text-museum-red' : 'text-gray-400 group-hover:text-museum-red transition' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                            Manajemen Koleksi
                        </a>
                        <a href="{{ route('educator.koleksi') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('educator.koleksi') ? 'bg-[#fbf7f5] text-museum-red font-semibold border-l-4 border-museum-red' : 'text-gray-500 hover:text-museum-red hover:bg-[#fbf7f5]' }} rounded-md transition font-medium group">
                            <svg class="w-5 h-5 {{ request()->routeIs('educator.koleksi') ? 'text-museum-red' : 'text-gray-400 group-hover:text-museum-red transition' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                            Koleksi
                        </a>
                        <a href="{{ route('educator.alat_edukasi') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('educator.alat_edukasi') ? 'bg-[#fbf7f5] text-museum-red font-semibold border-l-4 border-museum-red' : 'text-gray-500 hover:text-museum-red hover:bg-[#fbf7f5]' }} rounded-md transition font-medium group">
                            <svg class="w-5 h-5 {{ request()->routeIs('educator.alat_edukasi') ? 'text-museum-red' : 'text-gray-400 group-hover:text-museum-red transition' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                            Alat Edukasi
                        </a>
                ...
                @else
                    <a href="{{ route('registrar.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('registrar.dashboard') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} rounded-md text-sm">
                        <!-- Ikon Home -->
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                        Dasbor Utama
                    </a>
                    <a href="{{ route('registrar.create') }}" class="flex items-center gap-3 px-4 py-2.5 {{ request()->routeIs('registrar.create') ? 'bg-gray-50 text-black font-semibold border-l-4 border-black' : 'text-gray-600 font-medium hover:bg-gray-50' }} mt-1 rounded-md text-sm">
                        <!-- Ikon Edit -->
    ...
    <div class="flex-1 flex flex-col h-screen min-w-0">
        <!-- TOPBAR -->
        <header class="h-16 bg-[#fdfbf9] border-b border-[#f0e6dd] flex items-center justify-between px-8 shrink-0">
            @if(Auth::check() && Auth::user()->peran === 'admin')
                <h2 class="text-2xl font-serif text-[#5c1c16] font-medium">Management System</h2>
            @else
